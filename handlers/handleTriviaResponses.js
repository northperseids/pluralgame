...
module.exports = async (interaction, conn, botmessage, question, gameid, timers, client, answer) => {
    const prom = new Promise(async (resolve) => {
        ...
        collector.on('end', async collected => {
            ...
            submissions.forEach(submitter => {
                sql = "UPDATE players SET points=points+? WHERE playername=? AND gameid=?";
                conn.query(sql, [submitter.tally, submitter.playername, gameid]);
            });
            // specify winner and add extra point for winner?
            winners.forEach(winner => {
                sql = "UPDATE players SET points=points+? WHERE playername=? AND gameid=?";
                conn.query(sql, [1, winner.playername, gameid]);
                let s = "";
                if (winner.tally > 1) {
                    s = "s";
                }
